...
class exemption_form extends moodleform {
    ...
    public function definition() {
        ...
        if (empty($id)) {
            ...
            // The site context can have a lot of users, so we need to use AJAX to load the user list.
            if (count($users) > 1000 && $this->_customdata['contextid'] == context_system::instance()->id) {
    ...
    public function process(\stdClass $data): void {
        if (empty($data->id)) {
            foreach ($data->users as $userid) {
                penalty_exemption::exempt_user('gradepenalty_duedate', $userid, $this->_customdata['contextid'],
                    $data->reason['text'], $data->reason['format']);
            }

            foreach ($data->groups as $groupid) {
                penalty_exemption::exempt_group('gradepenalty_duedate', $groupid, $this->_customdata['contextid'],
                    $data->reason['text'], $data->reason['format']);
            }
        } else {
    ...
    private function get_participants() {
        ...
        $options = [];
        // for ($i = 1; $i <= 10000; $i++) {
        //     $options[$i] = $OUTPUT->render_from_template('gradepenalty_duedate/fullname_email', [
        //         'fullname' => "Fake User {$i}",
        //         'email' => "fakeuser{$i}@example.com",
        //     ]);
        // }
        foreach ($users as $user) {
            $options[$user->id] = $OUTPUT->render_from_template('gradepenalty_duedate/fullname_email', [
    ...
    public function self_populate() {
        ...
        switch ($exemption->get_itemtype()) {
            case penalty_exemption::TYPE_USER:
                ...
                $data->users = $OUTPUT->render_from_template('gradepenalty_duedate/fullname_email', [
                    'fullname' => fullname($user),
                    'email' => $user->email,
                ]);
                break;
