...
/**
 * Test class covering the user_exemption_service within the service layer of exemptions.
 *
...
class user_exemption_service_test extends \advanced_testcase {
    ...
    protected function get_mock_repository(array $mockstore) {
        ...
        $mockrepo->expects($this->any())
            ...
            ->will($this->returnCallback(function(exemption $exemption) use (&$mockstore) {
                ...
                $key = $exemption->userid . $exemption->component . $exemption->itemtype . $exemption->itemid
                    . $exemption->contextid;
    ...
    /**
     * Test getting a user_exemption_service from the static locator.
     */
    public function test_get_service_for_user_context(): void {
        list($user1context, $user2context, $course1context, $course2context) = $this->setup_users_and_courses();
        $userservice = \core_exemptions\service_factory::get_service_for_user_context($user1context);
        $this->assertInstanceOf(\core_exemptions\local\service\user_exemption_service::class, $userservice);
    }

    /**
     * Test confirming an item can be exemptiond only once.
     */
    public function test_create_exemption_basic(): void {
        list($user1context, $user2context, $course1context, $course2context) = $this->setup_users_and_courses();

        // Get a user_exemption_service for a user.
        ...
        // exemption a course.
        $exemption1 = $user1service->create_exemption('core_course', 'course', $course1context->instanceid, $course1context);
        $this->assertObjectHasProperty('id', $exemption1);

        // Try to exemption the same course again.
        $this->expectException('moodle_exception');
        $user1service->create_exemption('core_course', 'course', $course1context->instanceid, $course1context);
    }

    /**
     * Test confirming that an exception is thrown if trying to exemption an item for a non-existent component.
     */
    public function test_create_exemption_nonexistent_component(): void {
        list($user1context, $user2context, $course1context, $course2context) = $this->setup_users_and_courses();

        // Get a user_exemption_service for the user.
        $repo = $this->get_mock_repository([]); // Mock repository, using the array as a mock DB.
        $user1service = new \core_exemptions\local\service\user_exemption_service($user1context, $repo);

        // Try to exemption something in a non-existent component.
        $this->expectException('moodle_exception');
        $user1service->create_exemption('core_cccourse', 'my_area', $course1context->instanceid, $course1context);
    }
    ...
    public function test_find_exemptions_by_type_single_user(): void {
        ...
        // Get a user_exemption_service for the user.
        $repo = $this->get_mock_repository([]); // Mock repository, using the array as a mock DB.
        $service = new \core_exemptions\local\service\user_exemption_service($user1context, $repo);

        // exemption 2 courses, in separate areas.
        $exem1 = $service->create_exemption('core_course', 'course', $course1context->instanceid, $course1context);
        $exem2 = $service->create_exemption('core_course', 'anothertype', $course2context->instanceid, $course2context);

        // Verify we can get exemptions by area.
        $exemptions = $service->find_exemptions_by_type('core_course', 'course');
        ...
        $exemptions = $service->find_exemptions_by_type('core_course', 'anothertype');
        $this->assertIsArray($exemptions);
        $this->assertCount(1, $exemptions); // We only get exemptions for the 'core_course/course' area.
    ...
    public function test_find_all_exemptions(): void {
        ...
        $exem1 = $service->create_exemption('core_course', 'course', $course1context->instanceid, $course1context);
        $exem2 = $service->create_exemption('core_course', 'anothertype', $course2context->instanceid, $course2context);
        $exem3 = $service->create_exemption('core_course', 'yetanothertype', $course2context->instanceid, $course2context);

        // Verify we can get exemptions by area.
        $exemptions = $service->find_all_exemptions('core_course', ['course']);
        $this->assertIsArray($exemptions);
        $this->assertCount(1, $exemptions); // We only get exemptions for the 'core_course/course' area.
        $this->assertEquals($exem1->id, $exemptions[$exem1->id]->id);

        $exemptions = $service->find_all_exemptions('core_course', ['course', 'anothertype']);
        $this->assertIsArray($exemptions);
        ...
        $exemptions = $service->find_all_exemptions('core_course');
        $this->assertIsArray($exemptions);
    ...
    /**
     * Make sure the find_exemptions_by_type() method only returns exemptions for the scoped user.
     */
    public function test_find_exemptions_by_type_multiple_users(): void {
        list($user1context, $user2context, $course1context, $course2context) = $this->setup_users_and_courses();

        // Get a user_exemption_service for 2 users.
        $repo = $this->get_mock_repository([]);
        $user1service = new \core_exemptions\local\service\user_exemption_service($user1context, $repo);
        $user2service = new \core_exemptions\local\service\user_exemption_service($user2context, $repo);

        // Now, as each user, exemption the same course.
        ...
        // Verify find_exemptions_by_type only returns results for the user to which the service is scoped.
        $user1exemptions = $user1service->find_exemptions_by_type('core_course', 'course');
        $this->assertIsArray($user1exemptions);
        $this->assertCount(1, $user1exemptions); // We only get exemptions for the 'core_course/course' area for $user1.
        $this->assertEquals($exem1->id, $user1exemptions[$exem1->id]->id);

        $user2exemptions = $user2service->find_exemptions_by_type('core_course', 'course');
        $this->assertIsArray($user2exemptions);
        $this->assertCount(1, $user2exemptions); // We only get exemptions for the 'core_course/course' area for $user2.
        $this->assertEquals($exem2->id, $user2exemptions[$exem2->id]->id);
    }

    /**
     * Test confirming that an exception is thrown if trying to get exemptions for a non-existent component.
     */
    public function test_find_exemptions_by_type_nonexistent_component(): void {
        list($user1context, $user2context, $course1context, $course2context) = $this->setup_users_and_courses();
        ...
        // Verify we get an exception if we try to search for exemptions in an invalid component.
        $this->expectException('moodle_exception');
        $service->find_exemptions_by_type('cccore_notreal', 'something');
    }
    ...
    public function test_find_exemptions_by_type_pagination(): void {
        ...
        foreach (range(1, 10) as $i) {
            $service->create_exemption('core_course', 'course', $i, $course1context);
        }

        // Verify we have 10 exemptions.
        $this->assertCount(10, $service->find_exemptions_by_type('core_course', 'course'));

        // Verify we get back 5 exemptions for page 1.
        $exemptions = $service->find_exemptions_by_type('core_course', 'course', 0, 5);
        $this->assertCount(5, $exemptions);

        // Verify we get back 5 exemptions for page 2.
        $exemptions = $service->find_exemptions_by_type('core_course', 'course', 5, 5);
        $this->assertCount(5, $exemptions);

        // Verify we get back an empty array if querying page 3.
        $exemptions = $service->find_exemptions_by_type('core_course', 'course', 10, 5);
    ...
    public function test_delete_exemption_basic(): void {
        ...
        $service->delete_exemption('core_course', 'course', $course1context->instanceid, $course1context);
    ...
    public function test_count_exemptions_by_type(): void {
        ...
        // exemption a course.
        $service->create_exemption('core_course', 'course', $course1context->instanceid, $course1context);

        $this->assertEquals(1, $service->count_exemptions_by_type('core_course', 'course', $course1context));

        // exemption another course.
        $service->create_exemption('core_course', 'course', $course2context->instanceid, $course1context);
        ...
        $service->create_exemption('core_course', 'course', $course2context->instanceid, $course2context);
    ...
    public function test_get_join_sql_by_type(): void {
        ...
        // Verify the exemption information is returned, but only for the exemptiond course.
