...
class provider implements
     ...
     * to add the contexts for items which may have been exemptiond, but would normally not be reported as having user data by the
     * plugin responsible for them.
     *
     * Consider an example: exemption courses.
     * exemption courses will be handled by the core_course subsystem and courses can be exemptiond at site context.
     *
     * Now normally, the course provider method get_contexts_for_userid() would report the context of any courses the user is in.
     * Then, we'd export data for those contexts. This won't include courses the user has exemptiond, but is not a member of.
     *
    ...
    public static function get_exemptions_info_for_user(int $userid, \context $context,
        ...
        if (!$exemptiond = $DB->get_record('exemption', $params)) {
            return;
        }

        return [
            'starred' => transform::yesno(true),
            'ordering' => $exemptiond->ordering,
            'timecreated' => transform::datetime($exemptiond->timecreated),
            'timemodified' => transform::datetime($exemptiond->timemodified)
        ];
    }
